...
function getAverageGoals(getFinalsCallback) {
    ...
    const totalGoals = finals.reduce((acc, match) => acc + match["Home Team Goals"] + match["Away Team Goals"], 0);
    const averageGoals = (totalGoals / (finals.length * 2)).toFixed(2);
    return parseFloat(averageGoals);
  }
  const averageGoals = getAverageGoals(getFinals);
  console.log(averageGoals); // Output: 2.67
